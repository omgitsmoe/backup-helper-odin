...
filtered_walker_create :: proc(path: string, matcher: Matcher) -> (w: Filtered_Walker, err: Filtered_Walker_Error) {
...
filtered_walker_walk :: proc(w: ^Filtered_Walker) -> (fi: Filtered_File_Info, ok: bool) {
    for {
        ...
        case .Regular, .Directory: {
            ...
            rel, err := os.get_relative_path(
                w.root, fi.fi.fullpath, context.allocator)
        ...
        case .Undetermined, .Symlink, .Named_Pipe, .Socket,
