...
filtered_walker :: proc(t: ^testing.T) {
    ...
    helpers.create_test_files(t, test_dir, []helpers.Test_File{
    ...
    Expected :: struct{ relative_path: string, status: ch.File_Status }
    ...
    for tt in tests {
        ...
        testing.expectf(
            t, len(tt.expected) == len(actual),
            "expected length %v, got %v", len(tt.expected), len(actual))
        for a, idx in actual {
            testing.expectf(
                t, tt.expected[idx].relative_path == a.relative_path,
                "[%v] expected relative_path %v, got %v",
                idx, tt.expected[idx].relative_path, a.relative_path)
        }
    }
}
...
filtered_walker_skips_dir :: proc(t: ^testing.T) {
    ...
    testing.expectf(
        ...
        "expected length %v, got %v", len(expected), len(actual))
    for a, idx in actual {
        testing.expectf(
            t, expected[idx].relative_path == a.relative_path,
            "[%v] expected relative_path %v, got %v",
            idx, expected[idx].relative_path, a.relative_path)
    }
}
